...
void CGUIDialogVideoSettings::SaveChoice()
{
  ...
  std::string tvShowName = CMediaSettings::Get().GetCurrentMadvrSettings().m_TvShowName;
  int currentRes = CMediaSettings::Get().GetCurrentMadvrSettings().m_Resolution;
  ...
  else if (selected > -1 )
  {
    ...
    { // reset the settings
      ...
      if (selected == MADVR_RES_EPISODES)
      {
        ...
        dspdb.CreateVideoSettings(-1, currentRes, tvShowName, CMediaSettings::Get().GetCurrentMadvrSettings());
      }
      else
      {
        dspdb.EraseVideoSettings(selected, selected, "");
        dspdb.CreateVideoSettings(selected, selected, "", CMediaSettings::Get().GetCurrentMadvrSettings());
      }
      dspdb.Close();
    }
  }
