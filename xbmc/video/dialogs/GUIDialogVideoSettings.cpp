...
void CGUIDialogVideoSettings::Save()
{
  ...
  { // reset the settings
    ...
    if (m_isMadvr)
    {
      ...
      dspdb.Close();
      CMediaSettings::GetInstance().GetDefaultMadvrSettings() = CMediaSettings::Get().GetCurrentMadvrSettings();
    }
#endif

    CMediaSettings::GetInstance().GetDefaultVideoSettings() = CMediaSettings::Get().GetCurrentVideoSettings();
